...
function fCalendar_ev_showClose() {
    ...
    $gymColumnClickArea.on("click", function() {
      ...
      if($_body.hasClass("modalidades")) {
        if(document.querySelector('.modalidades .page-current .content-horarios'))
...
function fCalendar_start() {
    $(".page-current .nav-table").each(function() {
        ...
        var $placeTableLines = $(".page-current ." + place + " .table-horarios tbody tr");

        tableHorariosBGcolor($placeTableLines, studioToSee, place);
        tableHorariosClearHoras($placeTableLines, studioToSee);

        var $placeTableLinesShow = $(".page-current ." + place + " .table-horarios."+ studioToSee +" tbody tr.show");
        ...
        $placeTableLinesShow.find("td").each(function(){
          ...
          if($this.find("a." + studioToSee).length > 1) {
            $this.find("a." + studioToSee).parent().addClass("double");
...
function fCalendar_ev_estudios() {
    ...
    $btn_estudios.on("click", function() {
        ...
        $this.addClass('on');
        $placeTable.removeClass("" + studioWas + "").addClass("" + studioToSee + "");

        $.doTimeout(200, function() {
          tableHorariosClearHoras($placeTableLines, studioToSee);
          tableHorariosBGcolor($placeTableLines, studioToSee, place);

          var $placeTableLinesShow = $(".page-current ." + place + ".open .table-horarios."+ studioToSee +" tbody tr.show");
...
function tableHorariosBGcolor($elem, classe, place) {
    ...
    $('.gym-column').each(function(i) {
        ...
        if (i % 2 === 0) {
            $this.find(".show:even td").css("background-color", "rgba(255,255,255,.10)");
...
function fCalendar_ev_modalidades() {
    ...
    if ( $_body.hasClass('mobile') && $('.page-current .page-toload').hasClass("horarios") ) return;
    //** HOVER
    $btn_modalidades.hover(function() {
        var $this = $(this);
                ...
                $thisAnim.each(function(i) {
                    ...
                    $this.velocity({
                        ...
                        translateY: 0
                            ...
                            duration: 1000,
                            easing: [0.075, 0.820, 0.165, 1.000],
                            //easing: [.71,.45,.82,.69],
                            // easing: [0,1.13,.25,.99],
                            //easing:"easeOutSine",
                            mobileHA: true,
...
function fFooter2c(do_footer2c) {
    ...
    $(".footer-2c .cell-rgt").on("mouseenter", function(event) {
        var $this = $(this);
        $(".footer-2c .cell-rgt .btn-arrow").doTimeout("fOver", 100, "addClass", "over");
    ...
    $(".footer-2c .cell-rgt").on("mouseleave", function(event) {
        ...
        $(".footer-2c .cell-rgt .btn-arrow").doTimeout("fOver", 0, "removeClass", "over");
        event.preventDefault();
    });
...
function formMarcarVisitaEvents() {
  ...
  $radioButton.on("click", function(){
    ...
    if($this.find(".radio-selection").hasClass("selected")){
      ...
      $('#form-parceiro input[data-check-input="' + inputChecked + '"]').attr('checked', false);
    }
    else {
      $this.find(".radio-selection").removeClass("unselected").addClass("selected");
      $('#form-parceiro input[data-check-input="' + inputChecked + '"]').attr('checked', true);
    }
  });
...
function newsletterForm() {
  ...
  $(document).on("click", "#subForm button", function() {
    ...
    $.ajax({
      data: $(".footer #subForm").serialize(),
      type: $(".footer #subForm").attr('method'),
      dataType: 'json',
      url: $(".footer #subForm").attr('action'),
      cache: false,
      contentType: 'application/json; charset=utf-8',
      success: function(data) {
        //console.log("sent", data);
...
function promosForm() {
  ...
  $(document).on("submit", "#form-promo", function(event) {
    ...
    var gim = $('.radio-buttons .radio-selection.selected').parent('li').find('.value').text();
...
function ginasioPage() {
  ...
  if ( !$_body.hasClass('mobile') )
    $gymVideoPromise_ = $gymVideo.play();
  ...
  var init = function() {
    ...
    if (!$_body.hasClass('mobile')) {
      if ($gymVideoPromise_ !== undefined) {
        $gymVideoPromise_.then(function() {
